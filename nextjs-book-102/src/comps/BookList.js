 ...
 * search state 에 변화가 발생하고,
 * BookList 에서는 변화된 search state 의 값을 사용하여
 * 도서정보를 Naver로부터 fetch 한 후 도서리스트 state 인
 * books 에 반영하는 일을 수행해야 한다.
 *
 * client 컴포넌트인 BookList 에서 naver_api 모듈의
 * getNaverBooks() 모듈의 함수를 호출하여 naver 로 부터 데이터를
 * 가져와야 한다.
 * 문제는 naver_api 가 server(use server) 모듈이다.
 *
 * client 컴포넌트에서 serer 모듈(또는 컴포넌트)의 함수를 호출하여
 * 뭔가 수행하려면
 * 반드시 useEffect() 로 wrapping 해야한다.
 */
