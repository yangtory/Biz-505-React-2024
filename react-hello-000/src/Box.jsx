// children 은 변수이면서 키워드이다.
/**
 * <Comps>값</Copms> 처럼 전달한 값은
 * 자식 컴포넌트에서 {children} 매개변수를 통해서 받는다.
 *
 * 일반적인 함수에서는 호출한 곳에서 보내는 데이터를 받는 창구를
 * 매개변수, argument 라고 한다.
 *
 * f = (a,b) => {
 *      console.log(a+b)
 ...
 * const Comps = ({a,b})=>{
 *      console.log(a+b)
 *      a=100; // 오류발생 props 는 절대 변경불가 readonly
 *      b=200; // 오류발생
 * }
 * 호출할때는 이름=값 형식으로 전달해야 한다.
 * Comps(a=3, b=4)
 *
 * React(JSX) 에서는 부모 컴포넌트로 부터 전달받는 변수를 props 라고 한다.
